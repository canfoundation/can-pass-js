...
export const signTx = (txId: string, userId: string): Promise<any> => {
  const url = `${CAN_PASS_SIGN_TX_URL}?txId=${txId}&userId=${userId}`;
  return new Promise((resolve, reject) => popup(url).then((data) => {
